...
const currentIndex = ref(getRandomInt(props.forms.length))
const selected = ref<{ person: Person; number: GrammaticalNumber } | null>(null)
const missedAnswers = ref<ParseForm[]>([])
...
const current = computed(() => {
  const form = props.forms[currentIndex.value]
  if (!form) {
...
function pick(person: Person, number: GrammaticalNumber) {
  ...
  recordQuestionOutcome(props.gameId, current.value.form, correct)
  if (!correct) {
    missedAnswers.value.push(current.value)
  }
}
...
function nextQuestion() {
  let newIndex = getRandomInt(props.forms.length)
  while (newIndex === currentIndex.value && props.forms.length > 1) {
    newIndex = getRandomInt(props.forms.length)
  }
...
<template>
  <main class="luo">
    <h3 class="luo__prompt">{{ prompt }}</h3>
    ...
    <div v-if="answered" class="luo__feedback">
      ...
      <p class="luo__parse">
        <b>{{ current.form }}</b>
        <template v-if="current.ending"> — ending <b>{{ current.ending }}</b>,</template>
        <template v-else> —</template>
        {{ personLabel(current.person) }} person
        {{ current.number }}
      </p>
      <p class="luo__gloss">
        <template v-for="(line, i) in current.glossLines" :key="i">
          <i>{{ line }}</i><br v-if="i < current.glossLines.length - 1" />
        </template>
      </p>
    ...
    <details v-if="missedAnswers.length" class="missed">
      ...
      <ul>
        <li v-for="(m, i) in missedAnswers" :key="i">
          ...
          <template v-if="m.ending">(ending <b>{{ m.ending }}</b>)</template>
        </li>
      </ul>
    </details>
  </main>
